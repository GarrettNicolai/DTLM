...
void phraseModel::readEmbFile(param &myParam, string filename, hash_string_vectorDouble& Embeddings)
{
	vector<string> lineList;
	cout << "Reading embeddings file: " << filename << endl;
	...
	INPUTFILE.open(filename.c_str());
		...
		for(std::vector<int>::size_type i = 0; i != lineList.size(); i++)
		{
			if(i == 0)
			{
				index = lineList[i];
			...
			{
				embedding.push_back(atof(lineList[i].c_str()));
				//cout << atof(lineList[i].c_str()) << endl;
			}
		}

		Embeddings[index] = embedding;
	}
	cout << "Done reading embedding file" << endl;
...
void phraseModel::readWCFile(param &myParam, string filename, hash_string_double& WLCounts)
{
	...
	while (! INPUTFILE.eof())
	{
		...
		count /= normalizer;
			...
			string suffix = utf8_substr(parts[1], trueLength - i - 1, i);
			
			//cout << "WORD:" << parts[1] << endl;
		 	if(WLCounts.find(suffix) == WLCounts.end())
			{
			//	WLCounts[suffix] = 0.0;
			}
			...
			{
			//	WLCounts[suffix] = WLCounts[suffix] + (count * (i / trueLength));
                        }
			//cout << "WORD:" << parts[1] << endl;
	...
	/*
	int count;
	double probability1 = WLCounts["<w>pious<\\w>"];
	double probability2 = WLCounts["<w>piece<\\w>"];
	double probability3 = WLCounts["<w>pierce<\\w>"];
	...
	cout << "pious: " << str1.str() << endl;
	cout << "piece: " << str2.str() << endl;
...
void phraseModel::readLMFile(param &myParam, string filename, hash_string_double& LMProbs, hash_string_double& LMBackoff, int& maxLM)
{
	...
	int count;
	double probability1 = getLMProbability("pies", count, LMProbs, LMBackoff, maxLM, true);
	double probability2 = getLMProbability("piers", count, LMProbs, LMBackoff, maxLM, true);
	double probability3 = getLMProbability("pierce", count, LMProbs, LMBackoff, maxLM, true);
	double probability4 = getLMProbability("piuss", count, LMProbs, LMBackoff, maxLM, true);
...
void phraseModel::readingAlignedFile(param &myParam, string filename, hash_string_vData& output, bool genFea)
{
	...
	while (! INPUTFILE.eof())
	{
		...
		else
		{
			...
			stringstream ss;
			ss << output.size();
			string unAlignedWord = join(lineData.unAlignedX, "", "");// + ss.str(); 
			//string unAlignedWord = join(lineData.unAlignedX, "", ""); 
...
double phraseModel::getWCFeatureScore(param &myParam, hash_string_double& WLCounts, string word)
{
	...
	count /= parts.size();
        ...
        for(int j = atoi(bin.c_str()); j < 15; j++)
        {
...
double phraseModel::getLMFeatureScore(param &myParam, double LMScore)
{
	...
	string bin = getLMBin(LMScore);
        ...
        for(int j = atoi(bin.c_str()); j < 20; j++)
        {
             stringstream converter;
             converter << j;
             string jString = converter.str();
...
double phraseModel::getHigherOrderFeatureScore(param &myParam, vector_str observation, vector_str jointY, string c_class, bool flip)
{
	...
	//to getJointFeatureScore.
        ...
        if (myParam.markovOrder <= 0)
                return 0;
	if(flip && !jointY.empty())
        {
                vector_str flippedY;
                flippedY.push_back(c_class);
                ...
                while (!jointY.empty())
                {
                     flippedY.push_back(jointY.back());
                     jointY.pop_back();
...
double phraseModel::getJointGramFeatureScore(param &myParam, vector_str jointX, vector_str jointY, string currentX, string currentY, bool flip)
{
	double output = 0;
 	...
 	if(flip && !jointY.empty())
	{
		vector_str flippedX;
		vector_str flippedY;
		flippedX.push_back(currentX);
		flippedY.push_back(currentY);


		
		currentX = jointX.back();
		currentY = jointY.back();
		jointX.pop_back();
		jointY.pop_back();

		while (!jointX.empty())
  		{
    		     flippedX.push_back(jointX.back());
		     jointX.pop_back(); 
    		     flippedY.push_back(jointY.back());
		     jointY.pop_back(); 
		...
		//jointX = flippedX;
		//jointY = flippedY;
		//std::reverse(jointX.begin(), jointX.end());
		//std::reverse(jointY.begin(), jointY.end());
	}
...
vector_2str phraseModel::phrasalDecoder_beam(param &myParam, vector_str unAlignedX, vector_str unAlignedY,
                                                                                         ...
                                                                                         vector<double> &scoreNbest, hash_string_double& WLCounts, hash_string_double& LMProbs, hash_string_double& LMBackoff, int maxLM, bool flip)
{
	...
	for (int i = 0; i < unAlignedX.size(); i++)
	{
		...
		for (int k = 1; k <= myParam.maxX ; k++)
		{
			...
			if(flip)
			{
				std::reverse(focus.begin(), focus.end());
			}
			...
			if(flip)
			{
				std::reverse(lContext.begin(), lContext.end());
				std::reverse(rContext.begin(), rContext.end());
				vector_str temp = lContext;
				lContext = rContext;
				rContext = temp;
			}
			...
			for (vector<string>::iterator c_pos = allCandidate.begin(); c_pos != allCandidate.end(); c_pos++)
			{
				...
				if (i == 0)
				{
					...
					transScore = getHigherOrderFeatureScore(myParam, featureStr, Btmp.jointY, *c_pos, flip);
					jointMScore = getJointGramFeatureScore(myParam, Btmp.jointX, Btmp.jointY, Btmp.currentX, Btmp.currentY, flip);
					...
					if (myParam.WCInFilename != "")
					{
						...
						if(!flip)
						{
							generated = "<w>" + join(Btmp.jointY,"","") + Btmp.currentY;
						...
						{
							vector_str flippedY;	
							flippedY.push_back(Btmp.currentY);
							for(vector_str::reverse_iterator rit = Btmp.jointY.rbegin(); rit != Btmp.jointY.rend(); ++rit)
							{
								flippedY.push_back(*rit);		
							}
							generated = join(flippedY,"","") + Btmp.currentY;
							generated += "<\\w>";
                                               		removeSubString(generated, myParam.inChar); // remove inChar
                                               		removeSubString(generated, "_");
                                               		removeSubString(generated, "+");
							//cout << "X: " << join(unAlignedX, "", "") << endl;
							//cout << "Gen: " << generated << endl;
							//cout << "Joint: " << join(Btmp.jointX,"","") << " " << join(Btmp.jointY, "", "") << endl;
						}
						//cout << generated << endl;
						if(endOfWord)
						{
						...
						if(!flip)
						{
                                                	generated = join(Btmp.jointY,"","") + Btmp.currentY;
                                                	removeSubString(generated, myParam.inChar); // remove inChar
                                                	removeSubString(generated, "_");
							removeSubString(generated, "+");	
						}
					...
					for (D_btable::iterator p_pos = beamTable[i-1].begin(); p_pos != beamTable[i-1].end() ; p_pos++)
					{
						...
						//Turkish vowel harmony only concerns the suffixes
						{
							...
							if(!flip)
							{
								...
								oldGenerated = "<w>" + join(Btmp.jointY,"","");
                                                	...
                                                	else
                                                	{
                                                	        ...
                                                	        removeSubString(generated, "_");
                                                	        removeSubString(generated, "+");
								flippedY.erase(flippedY.begin());
								oldGenerated = join(flippedY, "","");
                                                	        removeSubString(oldGenerated, myParam.inChar); // remove inChar
                                                	        removeSubString(oldGenerated, "_");
                                                	        removeSubString(oldGenerated, "+");
						...
						if (myParam.LMInFilename != "")
							...
							oldGenerated = join(Btmp.jointY, "", "");//Have to remove old score, or LM gets too much weight
                                                       	removeSubString(oldGenerated, myParam.inChar); // remove inChar
                                                       	removeSubString(oldGenerated, "_");
							removeSubString(oldGenerated, "+");
	...
	for (int k =0 ; (k < myParam.nBest) && (beamTable[unAlignedX.size() - 1].size() > 0) ; k++)
	{
		...
		vector_str flippedX;
                vector_str flippedY;
                flippedX.push_back(beamTable[unAlignedX.size() - 1][0].currentX);
                flippedY.push_back(beamTable[unAlignedX.size() - 1][0].currentY);



                while (flip && !beamTable[unAlignedX.size() -1][0].jointX.empty())
                {
                     flippedX.push_back(beamTable[unAlignedX.size() -1][0].jointX.back());
                     beamTable[unAlignedX.size() -1][0].jointX.pop_back();
                     flippedY.push_back(beamTable[unAlignedX.size() -1][0].jointY.back());
                     beamTable[unAlignedX.size() -1][0].jointY.pop_back();
		...
		if(flip)
		{
			tempBestOutput = flippedY;
			tempBestAlignedX = flippedX;
			//cout << "Out: " << join(tempBestOutput, "", "") << endl;
		}
...
double phraseModel::minEditDistance(vector<string> str1, vector<string> str2, bool emb, hash_string_vectorDouble& Embeddings, string ignoreString)
{
	double distanceRate;
	double maxLength;
	double cost;
...
//	string str1,str2;
	...
	for (unsigned int i = 0; i < str1.size(); i++)
	{
		for (unsigned int j = 0; j < str2.size(); j++)
		{
			...
			{
				vector<double> A;// = Embeddings[str1[i]];
				vector<double> B;// = Embeddings[str2[j]];
				if(Embeddings.find(str1[i]) == Embeddings.end())
				{
					cost = 1;
				}
				else if(Embeddings.find(str2[j]) == Embeddings.end())
				{
					cost = 1;
				}
				else
				{
					A = Embeddings[str1[i]];
					B = Embeddings[str2[j]];
					cost = (1 - cosine_similarity_vectors(A, B));
					//cout << "A: " << str1[i] << " B: " << str2[j] << " Cost: " << cost << endl;
				}
			}
	...
	for (int i = 0; i < alignedTarget.size(); i++)
	{
		...
		if (featureList.size() > 0)
		{
			...
			//{
				...
				score += getHigherOrderFeatureScore(myParam, featureList[i], tmp, alignedTarget[i], false);
			//}
		}
		...
		if (myParam.jointMgram > 0)
		{
			...
			score += getJointGramFeatureScore(myParam, jointX, jointY, alignedSource[i], alignedTarget[i], false);
...
void phraseModel::training(param& myParam)
{
	...
	hash_string_double WLCounts;
        hash_string_vectorDouble Embeddings;
        int maxLM = 0;
	...
	if (myParam.EmbInFilename != "")
	{
		readEmbFile(myParam, myParam.EmbInFilename, Embeddings);
	}
	...
	while (stillTrain)
	{
		...
		for (hash_string_vData::iterator train_pos = trainUnique.begin(); train_pos != trainUnique.end(); train_pos++)
		{
			...
			vector<double> scoreNbest;
			vector_2str nBestAnswerFlip;
			vector_2str alignedXnBestFlip;
                        vector_3str featureNbestFlip;
                        vector<double> scoreNbestFlip;
			...
			if (myParam.useBeam)
			{
				...
				//vector_str flippedX(train_pos->second.mydata[0].unAlignedX.rbegin(), train_pos->second.mydata[0].unAlignedX.rend());
				//vector_str flippedY(train_pos->second.mydata[0].unAlignedY.rbegin(), train_pos->second.mydata[0].unAlignedY.rend());
				

                                nBestAnswer = phrasalDecoder_beam(myParam, train_pos->second.mydata[0].unAlignedX, train_pos->second.mydata[0].unAlignedY, alignedXnBest, featureNbest, scoreNbest, WLCounts, LMProbs, LMBackoff, maxLM, false);
				//nBestAnswerFlip = phrasalDecoder_beam(myParam, flippedX, flippedY, alignedXnBestFlip, featureNbestFlip, scoreNbestFlip, WLCounts, LMProbs, LMBackoff, maxLM, true);
			...
			/*for(int index = 0; index < nBestAnswerFlip.size(); index++)
			{
				cout << "Answer: " << join(nBestAnswerFlip[index], "", "") << endl;
			}*/
			//nBestAnswer.insert(nBestAnswer.end(), nBestAnswerFlip.begin(), nBestAnswerFlip.end());
			//alignedXnBest.insert(alignedXnBest.end(), alignedXnBestFlip.begin(), alignedXnBestFlip.end());
			//featureNbest.insert(featureNbest.end(), featureNbestFlip.begin(), featureNbestFlip.end());
			//scoreNbest.insert(scoreNbest.end(), scoreNbestFlip.begin(), scoreNbestFlip.end());
			...
			for (int nbi =0; nbi < nBestAnswer.size(); nbi++)
			{
				...
				if (myParam.alignLoss == "mulA")
				{
					...
					double pos_PER = minEditDistance(multipleRefs.mydata[0].alignedY, nBestAnswer[nbi], myParam.EmbInFilename != "", Embeddings, "_");
				...
				else if ((myParam.alignLoss != "minS") && (myParam.alignLoss != "maxS"))
				{
					...
					{
						double pos_PER = minEditDistance(multipleRefs.mydata[rti].alignedY, nBestAnswer[nbi], myParam.EmbInFilename != "", Embeddings, "_");
						/*std::string result = "";;
						std::string result2 = "";
			...
			long max_num_features =  (myParam.nGram + 1) * (myParam.nGram + 1) / 2 + myParam.copyFeature + myParam.finnishVH + myParam.turkishVH + myParam.turkishRH; //Add in an extra feature if copyFeature is true
                        if (myParam.LMInFilename != "")
                        {
                                max_num_features += 20; //Seven bins
                        }
		
			if (myParam.WCInFilename != "")
			{
				max_num_features += 15; //Nine bins
			}
			...
			} else if ((myParam.alignLoss == "minS") || (myParam.alignLoss == "maxS"))
			{
				...
				for (int nbi = 0; nbi < nBestAnswer.size(); nbi++)
				{
					...
					allNbestPER[nbi] = minEditDistance(multipleRefs.mydata[refForNBest[nbi]].alignedY, nBestAnswer[nbi], myParam.EmbInFilename != "", Embeddings, "_");
		...
		if (devUnique.size() > 0)
		{
			...
			//{
				...
				if (myParam.useBeam)
				{
					nBestAnswer = phrasalDecoder_beam(myDevParam, dev_pos->second.mydata[0].unAlignedX, dev_pos->second.mydata[0].unAlignedY, alignedXnBest, featureNbest, scoreNbest, WLCounts, LMProbs, LMBackoff, maxLM, false);
				}
				else
				{
				...
				{
					double pos_PER = minEditDistance(multipleRefs[rti].alignedY, nBestAnswer[0], myDevParam.EmbInFilename != "", Embeddings, "_");
	...
	if (myParam.testingFile == "")
	{
		...
		while (testword != "")
		{
			...
			if (myParam.useBeam)
			{
          			...
          			vector_str flippedX(unAlignedTestWord.rbegin(), unAlignedTestWord.rend());


                                nBestAnswer = phrasalDecoder_beam(myParam, unAlignedTestWord, nullY, alignedXnBest, featureNbest, scoreNbest, WLCounts, LMProbs, LMBackoff, maxLM, false);
                	//        nBestAnswerFlip = phrasalDecoder_beam(myParam, flippedX, nullY, alignedXnBestFlip, featureNbestFlip, scoreNbestFlip, WLCounts, LMProbs, LMBackoff, maxLM, true);
		...
		while (! INPUTFILE.eof())
		{
			...
			{*/
				...
				if (myParam.useBeam)
				{
					...
					nBestAnswer = phrasalDecoder_beam(myParam, test_pos->mydata[0].unAlignedX, nullY, alignedXnBest, featureNbest, scoreNbest, WLCounts, LMProbs, LMBackoff, maxLM, false);
				}
				else
				{
