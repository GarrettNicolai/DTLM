...
public:
	...
	void readLMFile(param &myParam, string filename, hash_string_double& LMProbs, hash_string_double& LMBackoff, int& maxLM);
 	void readEmbFile(param &myParam, string filename, hash_string_vectorDouble& Embeddings);
	...
	vector_2str phrasalDecoder(param& myParam, vector_str unAlignedX, vector_2str &alignedXnBest, vector_3str &featureNbest, vector<double>& scoreNbest);
 	...
 	vector_2str phrasalDecoder_beam(param& myParam, vector_str unAlignedX, vector_str unAlignedY, vector_2str &alignedXnBest, vector_3str &featureNbest, vector<double>& scoreNbest, hash_string_double& WLCounts, hash_string_double& LMProbs, hash_string_double& LMBackoff, int maxLM, bool flip);
...
, vector_str jointY, string c_class, bool flip);
        double getJointGramFeatureScore(param& myParam, vector_str jointX, vector_str jointY, string currentX, string currentY, bool flip);

	double getJointForwardGramFeatureScore(param& myParam, string currentX, string currentY, vector_str xForward);

	double minEditDistance(vector<string> str1, vector<string> str2, bool emb, hash_string_vectorDouble& Embeddings,  string ignoreString = "");
